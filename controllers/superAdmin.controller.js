...
const getAllSuperAdmins = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, count: superAdmins.length, data: superAdmins });
  } catch (err) {
    next(err);
  }
};
...
const updatePassword = async (req, res, next) => {
  try {
    const { superAdminId, oldPassword, newPassword, confirmPassword } =
      req.body;
...
// 📋 Get All Students under SuperAdmin
const getStudentsUnderSuperAdmin = async (req, res, next) => {
  try {
    const { superAdminId } = req.params;

    const superAdmin = await SuperAdmin.findById(superAdminId).populate({
      path: "students",
      populate: { path: "courseId", select: "name feesAmount duration" },
      options: { sort: { fname: 1 } },
    });
    ...
    res.status(200).json({
      success: true,
      count: superAdmin.students.length,
      data: superAdmin.students,
    });
  } catch (err) {
    next(err);
  }
